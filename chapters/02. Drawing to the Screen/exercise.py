...
class Browser:
    def __init__(self):
        ...
        # Linux: scrolling uses <Button-4> and <Button-5>
    ...
    def scroll_down(self, e):
        if self.scroll < self.page_height - HEIGHT:
    ...
    def draw_scrollbar(self):
        ...
        scrollbar_height = max(20, int(HEIGHT * visible_ratio))  # Min 20px

        max_scroll = self.page_height - HEIGHT
        scroll_ratio = self.scroll / max_scroll if max_scroll > 0 else 0
        available_space = HEIGHT - scrollbar_height
        scrollbar_y = int(scroll_ratio * available_space)
